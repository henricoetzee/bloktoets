...
function render_table(data, onclickfunction=false, clear_main=true, where="main_view") {
    ...
    for (row in data["data"]) {
        ...
        for (cell in data.data[row]) {
            // Skip certain names
            const skipped_columns = ["id", "stock_on_hand", "used_in_recipes", "unit_of_measure",
                                     "volume", "supplier_product_code", "list_cost", "last_received_cost"]
            if (!skipped_columns.includes(cell)) {
